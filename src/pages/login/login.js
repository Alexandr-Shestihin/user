...
//
import { isEmailValid, isFieldEmpty } from "../../helpers";
//
...
const Login = (props) => {
  ...
  useEffect(() => {
    ...
    if (userOnline) {
      ...
      if (props?.userData) {
        ...
        props.history.push(`/id/${props?.userData?.id}`);
      }
    }
    getData();
  ...
  const responseThirdAuth = (api, response) => {
    const { history } = props;
    console.log("responseThirdGoogleAuth", response);
    ...
    <StyledAuth>
      ...
      <form className="auth__form" noValidate autoComplete="off">
        ...
        <div className="btn__group">
          <ButtonLogin type="submit" onClick={routeChange} active={false}>
            <FormattedMessage id="login.signUpUppercase" />
          </ButtonLogin>
          <ButtonLogin
            type="submit"
            name="login"
